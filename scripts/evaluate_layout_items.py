...
def build_markdown_report(report_df: pd.DataFrame, summary: dict[str, Any]) -> str:
    ...
    lines.append("# Layout-aware Item Evaluation")
    lines.append("")
    lines.append("## Goal")
    lines.append("")
    lines.append(
        "Evaluate item extraction quality from `data/layout_extracted_results`, "
        "where receipt-level fields come from the rule-based parser and item rows "
